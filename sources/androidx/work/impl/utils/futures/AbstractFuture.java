...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
    ...
    public String toString() {
        ...
        if (isCancelled()) {
            ...
            if (strPendingToString != null && !strPendingToString.isEmpty()) {
                sbAppend.append("PENDING, info=[").append(strPendingToString).append(v8.i.e);
            } else if (isDone()) {
                addDoneString(sbAppend);
            } else {
        ...
        return sbAppend.append(v8.i.e).toString();
    }
    ...
    private void addDoneString(StringBuilder builder) {
        try {
            builder.append("SUCCESS, result=[").append(userObjectToString(getUninterruptibly(this))).append(v8.i.e);
        } catch (CancellationException unused) {
            builder.append("CANCELLED");
        } catch (RuntimeException e) {
            builder.append("UNKNOWN, cause=[").append(e.getClass()).append(" thrown from get()]");
        } catch (ExecutionException e2) {
            builder.append("FAILURE, cause=[").append(e2.getCause()).append(v8.i.e);
        }
    }
