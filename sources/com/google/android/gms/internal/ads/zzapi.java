...
public final class zzapi {
    public static X509Certificate[][] zza(String str) throws SecurityException, IOException, zzapf {
        RandomAccessFile randomAccessFile = new RandomAccessFile(str, AndroidDynamicDeviceInfoDataSource.DIRECTORY_MODE_READ);
        try {
            Pair pairZzc = zzapj.zzc(randomAccessFile);
            if (pairZzc == null) {
            ...
            try {
                ...
                while (byteBufferSlice.hasRemaining()) {
                    ...
                    if (i3 > byteBufferSlice.remaining()) {
                        throw new zzapf("APK Signing Block entry #" + i + " size out of range: " + i3 + ", available: " + byteBufferSlice.remaining());
                    }
                    if (byteBufferSlice.getInt() == 1896449818) {
                        X509Certificate[][] x509CertificateArrZzl = zzl(randomAccessFile.getChannel(), new zzape(zze(byteBufferSlice, i3 - 4), jLongValue2, jZza, jLongValue, byteBuffer, null));
    ...
    private static void zzh(Map map, FileChannel fileChannel, long j, long j2, long j3, ByteBuffer byteBuffer) throws SecurityException {
        ...
        zzapc zzapcVar = new zzapc(fileChannel, 0L, j);
        zzapc zzapcVar2 = new zzapc(fileChannel, j2, j3 - j2);
        ByteBuffer byteBufferDuplicate = byteBuffer.duplicate();
        byteBufferDuplicate.order(ByteOrder.LITTLE_ENDIAN);
        zzapj.zzd(byteBufferDuplicate, j);
        zzapa zzapaVar = new zzapa(byteBufferDuplicate);
        int size = map.size();
        ...
        try {
            byte[][] bArrZzk = zzk(iArr, new zzapb[]{zzapcVar, zzapcVar2, zzapaVar});
    ...
    private static byte[][] zzk(int[] iArr, zzapb[] zzapbVarArr) throws DigestException {
        ...
        while (true) {
            ...
            jZza += (zzapbVarArr[i4].zza() + 1048575) / PlaybackStateCompat.ACTION_SET_CAPTIONING_ENABLED;
            i4++;
        }
        if (jZza >= 2097151) {
        ...
        while (i8 < i) {
            ...
            while (jZza2 > 0) {
                ...
                try {
                    zzapbVar.zzb(messageDigestArr, j2, iMin);
                    int i13 = 0;
