package com.json;

import android.os.Handler;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import com.json.ft;
import com.json.mediationsdk.IronSource;
import com.json.mediationsdk.p;
import com.json.mediationsdk.testSuite.TestSuiteActivity;
import com.json.ns;
import com.vungle.ads.internal.ui.AdActivity;
import java.util.Iterator;
import java.util.List;
import kotlin.Metadata;
import kotlin.collections.CollectionsKt;
import kotlin.jvm.internal.Intrinsics;
import kotlin.text.StringsKt;
import org.json.JSONException;
import org.json.JSONObject;

/* JADX INFO: loaded from: classes3.dex */
@Metadata(d1 = {"\u0000Z\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u000e\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010 \n\u0002\u0010\u0000\n\u0002\b\u0005\n\u0002\u0018\u0002\n\u0002\b\u0005\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0010\u000b\n\u0002\b\u0005\n\u0002\u0018\u0002\n\u0002\b\u0007\u0018\u00002\u00020\u0001B)\u0012\u0006\u0010\u0003\u001a\u00020\u0002\u0012\u0006\u0010\u0005\u001a\u00020\u0004\u0012\u0006\u0010*\u001a\u00020\u001d\u0012\b\u0010\u0007\u001a\u0004\u0018\u00010\u0006¢\u0006\u0004\b+\u0010,J\"\u0010\t\u001a\u00020\b2\u0006\u0010\u0003\u001a\u00020\u00022\u0006\u0010\u0005\u001a\u00020\u00042\b\u0010\u0007\u001a\u0004\u0018\u00010\u0006H\u0003J\b\u0010\u000b\u001a\u0004\u0018\u00010\nJ\u0006\u0010\f\u001a\u00020\nJ(\u0010\t\u001a\u00020\b2\u0006\u0010\r\u001a\u00020\u00062\u0006\u0010\u000f\u001a\u00020\u000e2\u000e\u0010\u0012\u001a\n\u0012\u0006\u0012\u0004\u0018\u00010\u00110\u0010H\u0016J\u000e\u0010\t\u001a\u00020\b2\u0006\u0010\u0013\u001a\u00020\u0006J\u001e\u0010\t\u001a\u00020\u00062\u0006\u0010\u0014\u001a\u00020\u00062\u000e\u0010\u0015\u001a\n\u0012\u0006\u0012\u0004\u0018\u00010\u00110\u0010J\u0006\u0010\u0016\u001a\u00020\bJ\u0006\u0010\t\u001a\u00020\bR\u0014\u0010\u0019\u001a\u00020\u00178\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\t\u0010\u0018R\u0018\u0010\u001b\u001a\u0004\u0018\u00010\n8\u0002@\u0002X\u0082\u000e¢\u0006\u0006\n\u0004\b\u0016\u0010\u001aR\u0014\u0010\u001c\u001a\u00020\n8\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\f\u0010\u001aR\u0014\u0010\u001f\u001a\u00020\u001d8\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\u000b\u0010\u001eR\u0016\u0010#\u001a\u00020 8\u0002@\u0002X\u0082\u000e¢\u0006\u0006\n\u0004\b!\u0010\"R\u0016\u0010%\u001a\u00020 8\u0002@\u0002X\u0082\u000e¢\u0006\u0006\n\u0004\b$\u0010\"R\u0014\u0010)\u001a\u00020&8\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b'\u0010(¨\u0006-"}, d2 = {"Lcom/ironsource/ft;", "Lcom/ironsource/ss;", "Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;", "activity", "Lcom/ironsource/qg;", "uiLifeCycleListener", "", "controllerUrl", "", "a", "Landroid/webkit/WebView;", "d", "c", "callbackName", "Lcom/ironsource/mediationsdk/IronSource$AD_UNIT;", "adUnit", "", "", "args", "script", "methodName", "methodArgs", "b", "Landroid/os/Handler;", "Landroid/os/Handler;", "mHandler", "Landroid/webkit/WebView;", "mLoadingScreenWebView", "mAppWebView", "Lorg/json/JSONObject;", "Lorg/json/JSONObject;", "mJsonData", "", "e", "Z", "mInjectData", "f", "mLoadingScreenMinimumTimeExceeded", "Lcom/ironsource/ig;", "g", "Lcom/ironsource/ig;", "sdkConfigService", "jsonData", "<init>", "(Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;Lcom/ironsource/qg;Lorg/json/JSONObject;Ljava/lang/String;)V", "mediationsdk_release"}, k = 1, mv = {1, 8, 0})
public final class ft implements ss {

    /* JADX INFO: renamed from: a, reason: collision with root package name and from kotlin metadata */
    private final Handler mHandler;

    /* JADX INFO: renamed from: b, reason: from kotlin metadata */
    private WebView mLoadingScreenWebView;

    /* JADX INFO: renamed from: c, reason: from kotlin metadata */
    private final WebView mAppWebView;

    /* JADX INFO: renamed from: d, reason: from kotlin metadata */
    private final JSONObject mJsonData;

    /* JADX INFO: renamed from: e, reason: from kotlin metadata */
    private boolean mInjectData;

    /* JADX INFO: renamed from: f, reason: from kotlin metadata */
    private boolean mLoadingScreenMinimumTimeExceeded;

    /* JADX INFO: renamed from: g, reason: from kotlin metadata */
    private final ig sdkConfigService;

    @Metadata(d1 = {"\u0000)\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u000e\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003*\u0001\u0000\b\n\u0018\u00002\u00020\u0001J\u001c\u0010\u0007\u001a\u00020\u00062\b\u0010\u0003\u001a\u0004\u0018\u00010\u00022\b\u0010\u0005\u001a\u0004\u0018\u00010\u0004H\u0016J \u0010\f\u001a\u00020\u00062\u0006\u0010\u0003\u001a\u00020\u00022\u0006\u0010\t\u001a\u00020\b2\u0006\u0010\u000b\u001a\u00020\nH\u0016¨\u0006\r"}, d2 = {"com/ironsource/ft$a", "Landroid/webkit/WebViewClient;", "Landroid/webkit/WebView;", "view", "", "url", "", "onPageFinished", "Landroid/webkit/WebResourceRequest;", AdActivity.REQUEST_KEY_EXTRA, "Landroid/webkit/WebResourceError;", "error", "onReceivedError", "mediationsdk_release"}, k = 1, mv = {1, 8, 0})
    public static final class a extends WebViewClient {
        final /* synthetic */ String b;

        a(String str) {
            this.b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(WebView view, String url) throws JSONException {
            if (!ft.this.mInjectData && url != null && StringsKt.startsWith$default(url, String.valueOf(this.b), false, 2, (Object) null)) {
                ft ftVar = ft.this;
                List<? extends Object> listListOf = CollectionsKt.listOf(ftVar.mJsonData);
                Intrinsics.checkNotNull(listListOf, "null cannot be cast to non-null type kotlin.collections.List<kotlin.Any?>");
                ftVar.a(ftVar.a("onDataReady", listListOf));
                os.f4430a.d();
                ft.this.mInjectData = true;
            }
            super.onPageFinished(view, url);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) throws JSONException {
            Intrinsics.checkNotNullParameter(view, "view");
            Intrinsics.checkNotNullParameter(request, "request");
            Intrinsics.checkNotNullParameter(error, "error");
            os.f4430a.a(error.toString());
            zs.f4888a.b("got error during TestSuite web controller loading " + error);
            super.onReceivedError(view, request, error);
        }
    }

    @Metadata(d1 = {"\u0000\u0011\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u0002\n\u0002\b\u0003*\u0001\u0000\b\n\u0018\u00002\u00020\u0001J\b\u0010\u0003\u001a\u00020\u0002H\u0016J\b\u0010\u0004\u001a\u00020\u0002H\u0016¨\u0006\u0005"}, d2 = {"com/ironsource/ft$b", "Lcom/ironsource/qg;", "", "onUIReady", "onClosed", "mediationsdk_release"}, k = 1, mv = {1, 8, 0})
    public static final class b implements qg {
        final /* synthetic */ qg b;

        b(qg qgVar) {
            this.b = qgVar;
        }

        /* JADX INFO: Access modifiers changed from: private */
        public static final void a(b this$0) {
            Intrinsics.checkNotNullParameter(this$0, "this$0");
            this$0.onUIReady();
        }

        @Override // com.json.qg
        public void onClosed() {
            this.b.onClosed();
        }

        @Override // com.json.qg
        public void onUIReady() {
            if (ft.this.mLoadingScreenMinimumTimeExceeded) {
                this.b.onUIReady();
            } else {
                ft.this.mHandler.postDelayed(new Runnable() { // from class: com.ironsource.ft$b$$ExternalSyntheticLambda0
                    @Override // java.lang.Runnable
                    public final void run() {
                        ft.b.a(this.f$0);
                    }
                }, 500L);
            }
        }
    }

    public ft(TestSuiteActivity activity, qg uiLifeCycleListener, JSONObject jsonData, String str) {
        Intrinsics.checkNotNullParameter(activity, "activity");
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "uiLifeCycleListener");
        Intrinsics.checkNotNullParameter(jsonData, "jsonData");
        this.mHandler = new Handler(activity.getMainLooper());
        this.mLoadingScreenWebView = new WebView(activity);
        this.mAppWebView = new WebView(activity);
        this.mJsonData = jsonData;
        this.sdkConfigService = el.INSTANCE.d().r();
        a(activity, uiLifeCycleListener, str);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(ft this$0, String script) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        Intrinsics.checkNotNullParameter(script, "$script");
        this$0.mAppWebView.loadUrl("javascript:" + script);
    }

    private final void a(TestSuiteActivity activity, final qg uiLifeCycleListener, String controllerUrl) {
        WebView webView = this.mLoadingScreenWebView;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.mAppWebView.getSettings().setJavaScriptEnabled(true);
        List<? extends IronSource.AD_UNIT> listMutableListOf = CollectionsKt.mutableListOf(IronSource.AD_UNIT.BANNER, IronSource.AD_UNIT.INTERSTITIAL);
        if (this.sdkConfigService.a() != null) {
            List<IronSource.AD_UNIT> listP = p.m().p();
            Intrinsics.checkNotNullExpressionValue(listP, "getInstance().legacyInitializedAdUnits");
            Iterator<T> it = listP.iterator();
            while (it.hasNext()) {
                listMutableListOf.remove((IronSource.AD_UNIT) it.next());
            }
        } else {
            listMutableListOf.clear();
        }
        this.mAppWebView.addJavascriptInterface(new rs(new ns.a().a(listMutableListOf, activity, this.mHandler), new b(uiLifeCycleListener), this), "TestSuiteBridge");
        this.mAppWebView.setWebViewClient(new a(controllerUrl));
        if (controllerUrl != null && controllerUrl.length() != 0) {
            this.mAppWebView.loadUrl(controllerUrl);
        }
        WebView webView2 = this.mLoadingScreenWebView;
        if (webView2 != null) {
            webView2.loadUrl("javascript:document.write(atob(''))");
        }
        this.mHandler.postDelayed(new Runnable() { // from class: com.ironsource.ft$$ExternalSyntheticLambda1
            @Override // java.lang.Runnable
            public final void run() {
                ft.e(this.f$0);
            }
        }, 2000L);
        this.mHandler.postDelayed(new Runnable() { // from class: com.ironsource.ft$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
                ft.a(uiLifeCycleListener);
            }
        }, 5000L);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(qg uiLifeCycleListener) {
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "$uiLifeCycleListener");
        uiLifeCycleListener.onUIReady();
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void e(ft this$0) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        this$0.mLoadingScreenMinimumTimeExceeded = true;
    }

    public final String a(String methodName, List<? extends Object> methodArgs) {
        Intrinsics.checkNotNullParameter(methodName, "methodName");
        Intrinsics.checkNotNullParameter(methodArgs, "methodArgs");
        String str = "TestSuiteJs." + methodName;
        String strB = et.f3948a.b(methodArgs);
        return (strB.length() > 0 ? new StringBuilder().append(str).append('(').append(strB).append(')') : new StringBuilder().append(str).append("()")).toString();
    }

    public final void a() {
        b();
        this.mAppWebView.destroy();
    }

    public final void a(final String script) {
        Intrinsics.checkNotNullParameter(script, "script");
        this.mHandler.post(new Runnable() { // from class: com.ironsource.ft$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
                ft.a(this.f$0, script);
            }
        });
    }

    @Override // com.json.ss
    public void a(String callbackName, IronSource.AD_UNIT adUnit, List<? extends Object> args) {
        Intrinsics.checkNotNullParameter(callbackName, "callbackName");
        Intrinsics.checkNotNullParameter(adUnit, "adUnit");
        Intrinsics.checkNotNullParameter(args, "args");
        if (this.mInjectData) {
            String string = adUnit.toString();
            Intrinsics.checkNotNullExpressionValue(string, "adUnit.toString()");
            a(a("onCallbackReceived", CollectionsKt.listOf(callbackName, string, args)));
        }
    }

    public final void b() {
        WebView webView = this.mLoadingScreenWebView;
        if (webView != null) {
            if (webView != null) {
                webView.destroy();
            }
            this.mLoadingScreenWebView = null;
        }
    }

    /* JADX INFO: renamed from: c, reason: from getter */
    public final WebView getMAppWebView() {
        return this.mAppWebView;
    }

    /* JADX INFO: renamed from: d, reason: from getter */
    public final WebView getMLoadingScreenWebView() {
        return this.mLoadingScreenWebView;
    }
}
