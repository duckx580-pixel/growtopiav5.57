...
public final class LongSaturatedMathKt {
    ...
    public static final long m5045saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM4962toLongimpl = Duration.m4962toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m5044checkInfiniteSumDefinedPjuGub4(j, j2, jM4962toLongimpl);
        }
        if (((jM4962toLongimpl - 1) | 1) == Long.MAX_VALUE) {
            return m5046saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM4962toLongimpl;
        return ((j ^ j3) & (jM4962toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m5044checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m4948isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m5046saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM4919divUwyO8pc = Duration.m4919divUwyO8pc(j2, 2);
        long jM4962toLongimpl = Duration.m4962toLongimpl(jM4919divUwyO8pc, durationUnit);
        return (1 | (jM4962toLongimpl - 1)) == Long.MAX_VALUE ? jM4962toLongimpl : m5045saturatingAddNuflL3o(m5045saturatingAddNuflL3o(j, durationUnit, jM4919divUwyO8pc), durationUnit, Duration.m4951minusLRDsOJo(j2, jM4919divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m5018getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m5017getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m4969unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m4952plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m4969unaryMinusUwyO8pc(infinityOfSign(j3));
        }
